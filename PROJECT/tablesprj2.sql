...
phone_no number(10),
Ad_id number(5),
constraint teacher_adid_fk foreign key(AD_ID) references Admin_Dept(AD_ID) ON DELETE CASCADE);
...
T_ID varchar(10),
Constraint ex_fk foreign key(T_ID) references teacher(T_ID) ON DELETE CASCADE);
...
Constraint subt_fk foreign key(T_ID) references teacher(T_ID)ON DELETE CASCADE);
...
Constraint hw_fk foreign key(T_ID) references teacher(T_ID) ON DELETE CASCADE);
...
Constraint cl_ad_fk foreign key(AD_ID) references Admin_Dept(AD_ID) ON DELETE CASCADE,
CONSTRAINT C_T_FK foreign key(t_id) references Teacher(t_id) ON DELETE CASCADE);
...
T_ID varchar(10),
Roll_No number(5),
Constraint mk_fk1 foreign key(T_ID) references teacher(T_ID) ON DELETE CASCADE,
Constraint mk_fk2 foreign key(Roll_No) references student(Roll_No) ON DELETE CASCADE);
